...
export default function FlowPage({ view }: { view?: boolean }): JSX.Element {
  ...
  useEffect(() => {
    ...
    // Use the same host as the current page, but backend port
    const currentHost = window.location.hostname;
    const backendPort = "7860"; // Backend port
    const protocol = window.location.protocol === "https:" ? "wss:" : "ws:";
    
    // Only connect if we're not in development mode with different ports
    if (window.location.port !== backendPort) {
      const websocketBackendUrl = `${currentHost}:${backendPort}`;
      
      try {
        const ws = new WebSocket(
          `${protocol}//${websocketBackendUrl}/api/v1/events/ws`,
        );
        
        ws.onopen = () => {
          console.log("[WebSocket] Connected to backend for real-time updates");
        };
